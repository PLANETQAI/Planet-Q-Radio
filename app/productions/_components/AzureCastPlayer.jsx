...
export default function AzurePlayerBot() {
  ...
  const fetchNowPlaying = useCallback(async () => {
    try {
      const res = await api.get(NOW_PLAYING_URL, { cache: "no-store" }); // Use api.get
      const data = res.data; // Access data from res.data
...
useEffect(() => {
  ...
  async function togglePlayPause() {
    if (!isAuthenticated) { // Check isAuthenticated from context
